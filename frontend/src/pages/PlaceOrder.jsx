...
const PlaceOrder = () => {
    ...
    <div className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
      ...
      <div className='flex flex-col gap-4 w-full sm:max-w-[480px]'>
         ...
         <div className='flex gap-3'>
             ...
             <input className='border border-gray-300 py-1.5 px-3.5 w-full' type="text" placeholder="LastName name"/>
         </div>
  ...
  <p className={`w-3.5 h-3.5 border rounded-full ${method === 'stripe' ? 'bg-blue-700' : ' '}`}></p>
  <img className="h-5 mx-4" src={assets.stripe_logo} alt="Stripe" />
  </div>

  {/* Razorpay */}
  <div onClick={()=>setMethod('razorpay')} className="flex items-center gap-3 border p-2 px-3 cursor-pointer">
  <p className={`w-3.5 h-3.5 border rounded-full ${method === 'razorpay' ? 'bg-blue-700' : ' '}`}></p>
  <img className="h-5 mx-4" src={assets.Razorpay_logo} alt="Razorpay" />
  </div>

  {/* Cash on Delivery */}
  <div onClick={()=>setMethod('cod')} className="flex items-center gap-3 border p-2 px-3 cursor-pointer">
    <p className={`w-3.5 h-3.5 border rounded-full ${method === 'cod' ? 'bg-blue-700' : ' '}`}></p>
